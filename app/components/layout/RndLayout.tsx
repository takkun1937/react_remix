...
import { LayoutContext } from '~/features/work/providers/workSpaceProvider';
...
export const RndLayout = (props: RndLayoutProps): JSX.Element => {
  ...
  const codeSpaceHeightRatioRef = useRef<number>(
    layoutContext.rndLayout.codeSpaceHeightRatio
  );

  // CodeSpaceの高さの割合を追跡
  useEffect(() => {
    codeSpaceHeightRatioRef.current =
      layoutContext.rndLayout.codeSpaceHeightRatio;
  }, [layoutContext.rndLayout.codeSpaceHeightRatio]);
  ...
  useEffect(() => {
    ...
    // ウィンドウのリサイズイベントを監視
    const handleResize = () => {
      layoutContext.handleCodeSpaceResize(
        window.innerHeight,
        codeSpaceHeightRatioRef.current
      );
    };
  ...
  return (
    <Rnd
      size={{
        width: '100%',
        height:
          layoutContext.rndLayout.windowHeight *
          layoutContext.rndLayout.codeSpaceHeightRatio,
      }}
      position={{
        x: 0,
